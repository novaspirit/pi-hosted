...
sudo docker run -d -p 9000:9000 -p 9443:9443 --name=portainer --logo https://pi-hosted.com/pi-hosted-logo.png --restart=always -v /var/run/docker.sock:/var/run/docker.sock -v portainer_data:/data portainer/portainer-ce:latest || error "Failed to execute newer version of Portainer!"
